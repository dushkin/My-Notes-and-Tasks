...
export default function SettingsDialog({ isOpen, onClose }) {
  ...
  const allSettingsDescriptors = useMemo(
    () => [
      ...
      // Update the reminders group section in SettingsDialog.jsx
      // Replace the existing reminders group object with this updated version:

      {
        ...
        settings: [
          ...
          {
            id: "reminderDisplayDoneButton",
            label: "Display Done Button",
            desc: "Show Done button in notifications and require user interaction. When disabled, notifications auto-dismiss after 5 seconds. Recommended for users with attention challenges.",
            control: (
              <input
                id="reminderDisplayDoneButton"
                type="checkbox"
                data-item-id="setting-reminderdonebutton-checkbox"
                checked={!!settings.reminderDisplayDoneButton}
                onChange={(e) =>
                  updateSetting("reminderDisplayDoneButton", e.target.checked)
                }
                className="form-checkbox h-5 w-5 text-blue-600 rounded cursor-pointer"
              />
            ),
