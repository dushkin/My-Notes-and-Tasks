...
// Accept defaultFontFamily and defaultFontSize props
const EditorPane = ({ html = "", onChange, defaultFontFamily, defaultFontSize }) => { // MODIFIED: Added props
  // Use defaults from props for initial state, falling back to hardcoded if props are missing
  const [fontFamily, setFontFamily] = useState(defaultFontFamily || FONT_FAMILIES[0]); // MODIFIED: Use prop default
  const [fontSize, setFontSize] = useState(defaultFontSize || FONT_SIZES[2]); // MODIFIED: Use prop default (index 2 is size '3')
  ...
  // Handle input, including delayed auto-linking
  ...
  // const findEquivalentNode = (root, targetNode) => { // <--- Defined earlier
  ...
  const handlePasteFromClipboard = useCallback(async () => { // 
    ...
    editorRef.current?.focus(); // 
  }, [onChange, applyCommand]); // // onChange is a dependency, Added applyCommand
