...
const ContextMenu = ({
  ...
  return (
    <div
      ref={contextMenuRef}
      className="fixed z-50 bg-white dark:bg-zinc-800 border border-zinc-500 rounded shadow-md text-sm min-w-[180px]"
      style={{ top: y, left: x }}
    >
      ...
      {isEmptyArea && (
        <>
          <button
            ...
            onClick={() => { onAddRootFolder(); onClose(); }}
          >
             ➕ Add Root Folder
          </button>
          <hr className="my-1 border-zinc-300 dark:border-zinc-600" />
           {/* Conditionally render Paste */}
           {canPaste && (
              <button
                className="block w-full px-4 py-2 text-left flex items-center gap-2 hover:bg-zinc-100 dark:hover:bg-zinc-700"
           ...
           {/* Only show separator if Paste was shown */}
           {canPaste && <hr className="my-1 border-zinc-300 dark:border-zinc-600" />}
           <button
            className="block w-full px-4 py-2 text-left hover:bg-zinc-100 dark:hover:bg-zinc-700 flex items-center gap-2"
            onClick={() => { onExportTree(); onClose(); }} // Use specific handler
           ...
           <button
            className="block w-full px-4 py-2 text-left hover:bg-zinc-100 dark:hover:bg-zinc-700 flex items-center gap-2"
            onClick={() => { onImportTree(); onClose(); }} // Use specific handler
      ...
      {!isEmptyArea && item && (
        <>
          ...
          {/* Conditionally render Paste only on folders and if canPaste */}
          ...
          <hr className="my-1 border-zinc-300 dark:border-zinc-600" />
           ...
           <button
              className="block w-full px-4 py-2 text-left hover:bg-zinc-100 dark:hover:bg-zinc-700 flex items-center gap-2"
              onClick={() => { onExportItem(); onClose(); }} // Use specific handler
           ...
           {item.type === 'folder' && (
             <button
                className="block w-full px-4 py-2 text-left hover:bg-zinc-100 dark:hover:bg-zinc-700 flex items-center gap-2"
                onClick={() => { onImportItem(); onClose(); }} // Use specific handler
