...
- Name: Search Functionality >> should find items by content and respect case sensitivity option
- Location: C:\Users\TalTe\Dev Projects\my-notes-and-tasks\tests\e2e\search.spec.ts:141:3
...
Locator: locator('[data-item-id="search-sheet-container"]').locator('div.p-3.sm\\:p-2.border-b').filter({ has: locator('p.text-base.md\\:text-sm').filter({ hasText: 'Pasta Recipe' }) })
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 7000ms
  - waiting for locator('[data-item-id="search-sheet-container"]').locator('div.p-3.sm\\:p-2.border-b').filter({ has: locator('p.text-base.md\\:text-sm').filter({ hasText: 'Pasta Recipe' }) })

    at C:\Users\TalTe\Dev Projects\my-notes-and-tasks\tests\e2e\search.spec.ts:160:7
```
...
- textbox "Search...": tomatoes
- button "Case Sensitive"
- button "Whole Word"
- button "Use Regular Expression (Disabled)" [disabled]
- button "Close Search"
- paragraph: Recipes / Pasta Recipe(content)
- paragraph:
  - text: "Ingredients: pasta,"
  - strong: tomatoes
  - text: ", garlic."
- paragraph: Shopping List(content)
- paragraph:
  - strong: Tomatoes
  - text: ", pasta, eggs, milk."
- button
```
...
> 160 |     ).toBeVisible({ timeout: 7000 });
  ...
  222 |     await expect(
  223 |       getSearchResultEntryLocator(page, searchSheet, "Project Plan")
  224 |     ).not.toBeVisible();
  225 |
  226 |     await searchSheet.getByRole("button", { name: "Whole Word" }).click();
  227 |     await searchInput.fill("pla");
  228 |     await expect(
  229 |       getSearchResultEntryLocator(page, searchSheet, "Project Plan")
  230 |     ).toBeVisible();
  231 |   });
  232 |
  233 |   test('should show "No matches" if search yields no results', async ({
  234 |     page,
  235 |   }) => {
  236 |     await page.getByRole("button", { name: "Search (Ctrl+Shift+F)" }).click();
  237 |     const searchSheet = getSearchSheetLocator(page);
  238 |     await expect(searchSheet).toBeVisible({ timeout: 10000 });
  239 |     const searchInput = searchSheet.locator("input#global-search-input");
  240 |     await expect(searchInput).toBeVisible();
  241 |
  242 |     await searchInput.fill("xyznonexistentquery123");
  243 |     await expect(searchSheet.getByText("No matches.")).toBeVisible();
  244 |   });
  245 |
  246 |   test("should close search sheet with close button", async ({ page }) => {
  247 |     await page.getByRole("button", { name: "Search (Ctrl+Shift+F)" }).click();
  248 |     const searchSheet = getSearchSheetLocator(page);
  249 |     await expect(searchSheet).toBeVisible({ timeout: 10000 });
  250 |
  251 |     await searchSheet.getByRole("button", { name: "Close Search" }).click();
  252 |     await expect(searchSheet).not.toBeVisible({ timeout: 5000 });
  253 |   });
  254 | });
  255 |
```